...
interface ElasticsearchPluginConfig {
  indexingCronSchedule?: string;
  searchConnector?: SearchConnectorConfig;
  [key: string]: unknown;
}

export default async ({ strapi }: { strapi: Core.Strapi }) => {
    const pluginConfig = strapi.config.get('plugin::elasticsearch') as ElasticsearchPluginConfig;
    ...
    try
    {
      ...
      {
        const connector = pluginConfig['searchConnector'] as SearchConnectorConfig;
