...
export default ({ strapi }: { strapi: Core.Strapi }) => ({
    ...
    async rebuildIndex(): Promise<boolean | undefined> {
        const helper = (strapi.plugins['elasticsearch'] as any).services.helper;
        const esInterface = (strapi.plugins['elasticsearch'] as any).services.esInterface;
        const scheduleIndexingService = (strapi.plugins['elasticsearch'] as any).services.scheduleIndexing;
        const configureIndexingService = (strapi.plugins['elasticsearch'] as any).services.configureIndexing;
        const logIndexingService = (strapi.plugins['elasticsearch'] as any).services.logIndexing;
    ...
    async indexCollection(collectionName: string, indexName: string | null = null) {
        const helper = (strapi.plugins['elasticsearch'] as any).services.helper;
    ...
    async indexPendingData() {
        ...
        const fullSiteIndexing = recs.filter((r: any) => r.full_site_indexing === true).length > 0
        if (fullSiteIndexing)
        {
