...
const Configure = () => {
  ...
  const [config, setConfig] = useState<ConfigDisplayItem[] | null>(null);
  ...
  const importContentConfig = (conf: string) => {
  ...
  const scheduleCollectionIndexing = (collectionName: string) => {
  ...
  useEffect(() => {
    setIsInProgress(true);
    loadContentConfig()
    .then((resp: Record<string, Record<string, {index?: boolean}>>) => {
      const displayConfig: ConfigDisplayItem[] = [];
      for (let r=0; r < Object.keys(resp).length; r++)
      {
        const item: ConfigDisplayItem = {collectionName: Object.keys(resp)[r], indexed: [], notIndexed: []};
